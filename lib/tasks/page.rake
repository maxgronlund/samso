namespace :page do
  ...
  task build_subscription_pages: :environment do
    ...
    pages_params = [
      { title: 'Abonnere', menu_title: 'Abonnere', menu_id: 'not_in_any_menus', locale: 'da', active: true, user_id: user.id },
      { title: 'Subscribe', menu_title: 'Subscribe', menu_id: 'not_in_any_menus', locale: 'en', active: true, user_id: user.id }
    ]
  ...
  task build_front_pages: :environment do
    ...
    pages_params = [
      ...
      { title: 'Front page', menu_title: 'SAMSØ', menu_id: 'not_in_any_menus', locale: 'en', active: true, user_id: user.id }
    ]
  ...
  task build_post_pages: :environment do
    user = User.super_admin
    pages_params = [
      { title: 'Artikel', menu_title: 'Artikel', menu_id: 'not_in_any_menus', locale: 'da', active: true, user_id: user.id },
      { title: 'Article', menu_title: 'Article', menu_id: 'not_in_any_menus', locale: 'en', active: true, user_id: user.id }
    ]
  ...
  task build_welcome_pages: :environment do
    user = User.super_admin
    pages_params = [
      { title: 'Velkommen', menu_title: '', menu_id: 'not_in_any_menus', locale: 'da', active: true, user_id: user.id },
      { title: 'Welcome', menu_title: '', menu_id: 'not_in_any_menus', locale: 'en', active: true, user_id: user.id }
    ]
    pages_params.each do |page_params|
      page = Page.where(page_params).first_or_create(page_params)
      admin_system_setup = Admin::SystemSetup.find_by(locale: page_params[:locale])
