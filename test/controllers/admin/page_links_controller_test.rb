...
class Admin::PageLinksControllerTest < ActionDispatch::IntegrationTest
  ...
  test 'should create admin_page_link' do
    assert_difference('Admin::PageLink.count') do
      post(
        admin_page_links_url,
        params: {
          admin_page_link: {
            background_color: @admin_page_link.background_color,
            color: @admin_page_link.color,
            name: @admin_page_link.name,
            page_id: @admin_page_link.page_id
          }
        }
      )
    end
  ...
  test 'should update admin_page_link' do
    patch(
      admin_page_link_url(@admin_page_link),
      params: {
        admin_page_link: {
          background_color: @admin_page_link.background_color,
          color: @admin_page_link.color,
