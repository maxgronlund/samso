...
# Take the input_field id as a parameter and pass it on
autofill_city_from_post_code = (post_code_field) ->
  parent_div = post_code_field.parents("fieldset, .clear")
  post_code_autofill_dk(post_code_field)


post_code_autofill_dk = (post_code_field) ->
  post_code = post_code_field.val()
  if post_code >= 100 and post_code <= 9999
    city_name = post_code_lookup(post_code_all['dk'], post_code)
    #console.log city_name_field
    $('.city-name-autofill').val(city_name)

# Do the actual lookup of post code to city name.
post_code_lookup = (post_code_array, post_code) ->
  city_name = post_code_array[post_code]
  if city_name is `undefined`
    post_code_array_keys = Object.keys(post_code_array)
    j = 0

    while j < post_code_array_keys.length
      key = post_code_array_keys[j]
      if post_code_array_keys[j].match("-")
        from_to = post_code_array_keys[j].split("-", 2)
        city_name = post_code_array[key] if from_to[0] <= post_code and from_to[1] >= post_code
      j++
  city_name

# Array of all post codes by country.
# May also contain an interval of post codes which map to same city name.
post_code_all =
  'dk':
